...
The integration is designed around three independent extension points:

### Strategies — easiest to extend
Add a new energy management strategy in **2 steps**:
1. Create `strategies/my_strategy.py` — subclass `BaseStrategy`, implement `execute(context)`
2. Add one line to `STRATEGY_MAP` in `strategies/__init__.py`

Nothing else needs to change. The coordinator, UI selector, sensor platform, and financial tracker all pick it up automatically.

### Battery devices — minimal touch points
Add a new battery brand in **3 steps**:
1. Create `devices/my_brand.py` — subclass `BatteryDevice`, implement 5 methods
2. Add a config step in `config_flow.py`
3. Register it in `coordinator.py` → `_build_devices()`

Financial tracking, sensor generation, and strategy compatibility are fully automatic for any new device.

### Financial tracking — fully automatic
The `FinancialTracker` handles any device by `device_id`. When a new device is registered in the coordinator it is automatically tracked across all periods (daily/weekly/monthly/yearly) and a full set of financial sensors is created for it — no changes needed in `financial/` or `sensor.py`.

### Current limitations
| Area | Status |
|---|---|
| Config flow | Hardcoded per device type — not a dynamic plugin registry yet |
| Device discovery | No auto-discovery; all devices must be explicitly configured |
| Energy measurement | Estimated from setpoint, not from real energy meters |
| Multi-instance | Single HEMS instance per HA installation |

---

## Dashboard

A ready-made Lovelace dashboard is included in `dashboards/hems_dashboard.yaml`.

**Import steps:**
1. Go to **Settings → Dashboards → Add Dashboard**
2. Give it a name (e.g. "HEMS")
3. Open the new dashboard → pencil icon → three dots → **Raw configuration editor**
4. Paste the contents of `hems_dashboard.yaml`

**Views included:**

| View | Contents |
|---|---|
| Overzicht | Strategy selector, live grid power gauge, P1 history graph, price history |
| Batterijen | Per device: kWh charged/discharged, daily/weekly/monthly/yearly profit, lifetime totals |
| Financiën | Combined glance overview, detailed kWh table, profit trend graphs |
| Grafieken | Placeholder view for enhanced charts (requires `apexcharts-card` from HACS) |

> **Note:** Entity IDs are auto-generated by HA based on entity names. If your IDs differ slightly, check **Developer Tools → States** and filter on `multi_battery_hems` to find the correct names, then update the YAML.

**Optional enhancement:** install `apexcharts-card` via HACS → Frontend for a live price bar chart (showing cheap/expensive hours) and multi-series battery power charts.
